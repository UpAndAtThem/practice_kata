...
# speak is an instance method of both Person and Dog.  Being ruby's look up path for speak the 2 will never have overriding issues because they are not in the same hierarcical structure.
# speak is defined within both classes, so when ruby first looks within the calling objects class for the method, it will invoke the first match it comes across. This is how polymorphism is achieved.
